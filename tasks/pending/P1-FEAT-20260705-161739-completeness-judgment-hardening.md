# PRD: iar 完成度判定结构性盲区加固（stdout 断言 + supervisor diff 分层 + 跨 cycle finding 累积）

> ✅ **交付前置**：无，可立即开工。
> 结构化声明见 §8 Delivery Dependencies，**那里是唯一事实源**。

> **2026-09-16 复核（相对初版的变化，开工前必读）**
> - **补丁 2（verifier 默认开 + telemetry）已移除**：`ValidationConfig.verifier_enabled` 的默认值已于
>   2026-07-07 由 `c3509c1` 改为 `True`（现状见 `src/backend/core/shared/models/agent_runner.py:532`
>   与 `src/backend/infrastructure/config/agent_runner_settings.py:252`），初版列出的"盲区 2"不再存在；
>   配套的 verifier telemetry 立项动机（观察默认值变更影响）随之消失。理由见 §13 D-11。
>   受影响的 FR-3 / FR-4 / FR-5、rv-3 与相应验收项已一并删除，**编号保留空缺以免与历史讨论错位**。
> - **路径与行号已按当前 main 刷新**（`5ae82c6` 拆分超长文件后原行号已漂移）：`_run_supervisor_with_repair_loop`
>   现位于 `agent_runner_supervisor.py:47`（初版记为 `pr_supervisor.py`）；`_rv_reexec_cache_relpath` 现为
>   `agent_runner_validation.py:458`（初版记 565-579）。完整锚点见 §5 与 §7.3。
> - **测试基线更新**：初版写"176+ 测试"，2026-09-16 实测 `uv run pytest tests/ -q -o addopts=""` 为 **2091 passed**。
...
iar（issue-agent-runner）当前在"agent 报告完成 → PR 进入人审"这条链路上曾有 4 个**结构性盲区**——任一被 agent 利用都会让 keda 自检通过、但 supervisor 仍能发现真实问题（即"完成度不足"的典型表现）。

初版撰写时（2026-07-05）的样本是 issue #4（`agent/blocked`）：agent 自检 RV 全绿、verification_commands exit 0，但 supervisor 第 3 轮仍报 1 high + 3 medium + 2 low，最终被 `max_repair_exceeded` 标 blocked。

初版列出的四个盲区（2026-09-16 复核状态）：

1. **RV 命令只看 exit code 不看 stdout**——agent 写 `[ -d x ] || true` 或 `grep ... || echo ok` 就能 exit 0 蒙混过关；真实断言从未被核验。**仍未修复，本 PRD 补丁 1 处理**
2. ~~**独立 verifier 默认关闭**——`verifier_enabled=False` 是出厂设置~~ **已于 2026-07-07 修复**（`c3509c1`，现状默认 `True`）；对应补丁已从本 PRD 移除，见 §13 D-11
3. **supervisor 看到的 PR diff 被截断到 6000 字符**——长 PR 的关键变更落在截断窗口外，supervisor LLM 看不全就 approve。**仍未修复，本 PRD 补丁 3 处理**
4. **supervisor 多 cycle 之间不累积 findings**——cycle 2 报的 high 没修，cycle 3 会"忘了"再检；`max_repair_attempts=2` 容易被打死。**仍未修复，本 PRD 补丁 4 处理**

这 4 个不是单点 bug，而是 iar 完成度判定机制的结构性缺陷：单条防线够硬，但**防线之间的协同**不够——agent 自检通过 ≠ supervisor 也确认通过 ≠ 真实功能实现。

### Interpretation (解读回显)

我把"完成度判定加固"读作：在 iar 自身（keda 仓）上落地 3 个**最小变更的针对性补丁**（补丁 1 / 3 / 4，编号沿用初版），不重构现有架构、不引入新依赖、不改状态机。每个补丁都基于已存在的扩展点（`EvidenceBlock` 字段、`PostPrSupervisorConfig`、`_run_supervisor_with_repair_loop`）延伸；不属于"重新设计完成度模型"那种大动作。
...
- ❌ 把 3 个补丁合并为一个抽象的"完成度引擎"模块

如果这个解读偏了，请在动工前指出。初版此处提到过"加 telemetry 但不改 verifier 默认值"的备选，该备选连同补丁 2 已于 2026-09-16 复核时移除（见 §13 D-11）。
...
2. **大 PR diff 不丢失**：构造一个含 5 个关键文件、diff 总长 15000 字符的 PR，supervisor prompt 中关键文件 diff 完整可见
3. **跨 cycle finding 累积**：构造一个 cycle 1 报 high、cycle 2 没修的剧本，cycle 2 supervisor prompt 包含 cycle 1 的未解决 high 列表
4. **现有 2091 测试不回归**（2026-09-16 实测）：`uv run pytest -o addopts="" tests/` 全绿；架构门禁 `hooks/shared/check_architecture.py` 绿
5. **零依赖变化**：`git diff pyproject.toml uv.lock` 为空
...
- ① Core 业务逻辑 / 编排规则：`ensure_validation_commands_pass`（补丁 1）、`build_supervisor_prompt`（补丁 3）、`run_post_pr_supervisor_cycle`（补丁 4）都在 `core/use_cases/` 下；这是 iar 完成度判定的核心机制，属于 fixed zone
  - 补丁 2 已于 2026-07-07 落地（`verifier_enabled` 默认 `True`），不再列入本 PRD 的风险地图
...
（`[validation].verifier_enabled` 的默认值变更已于 2026-07-07 随 `c3509c1` 落地，不再是本 PRD 的交付项。）
...
- **Scope boundary**:
  ...
  - 不强制要求所有下游项目立即启用新配置（补丁 2 移除后本 PRD 无默认值变更）
...
- **Existing path**（行号为 2026-09-16 复核时的 main 现状，与初版不同）：
  - 补丁 1：`src/backend/core/use_cases/agent_runner_validation.py:535` 的 `ensure_validation_commands_pass`
    + `src/backend/core/use_cases/agent_runner_structured_evidence.py:102` 的 `EvidenceBlock`
  - 补丁 3：`src/backend/core/use_cases/pr_supervisor.py:182` 的 `build_supervisor_prompt`
    + `src/backend/core/shared/models/agent_runner.py:584` 的 `PostPrSupervisorConfig`
    （6000 字符截断现状在 `pr_supervisor.py:238-240`）
  - 补丁 4：`src/backend/core/use_cases/pr_supervisor.py:831` 的 `run_post_pr_supervisor_cycle`
    + `src/backend/core/use_cases/agent_runner_supervisor.py:47` 的 `_run_supervisor_with_repair_loop`
    （初版记在 `pr_supervisor.py`，`5ae82c6` 拆分文件时已迁出）
- **Reuse candidates**:
  - `_extract_expected_artifacts` 模式（`agent_runner_structured_evidence.py:379`）——补丁 1 加 `stdout_assertions` 字段照抄
  - `_rv_reexec_cache_relpath` / `_rv_reexec_cache_path`（`agent_runner_validation.py:458` 起）——补丁 4 finding artifact 路径解析照抄
  - `parse_supervisor_action` fail-closed 解析（`pr_supervisor.py:308`）——补丁 4 扩展 `findings` 字段时遵循"先过 action 解析，再抽 findings"顺序
  - `format_validation_evidence_failure` 错误详情格式化（`agent_runner_validation.py:659`）——补丁 1 关键词失败时复用
...
- **Frontend impact**: **No frontend impact**（iar 本体是 CLI 后端工具；本 PRD 不触碰仓库内的
  `frontend-public/` 与 `frontend-admin/` 应用代码——这两个前端应用在初版撰写后已存在于仓库，
  初版"仓库无 frontend app"的表述已过时，但对本 PRD 的改动面没有影响）
...
- **Approach**: 在 3 个已存在的扩展点（`EvidenceBlock` / `PostPrSupervisorConfig` / supervisor cycle artifact）上**追加**字段和逻辑，不重写、不抽象、不并入新模块
...
- ~~**替代 A**（针对补丁 2，已随补丁 2 移除）~~：该补丁的"默认值变更"部分已于 2026-07-07 随 `c3509c1` 落地
...
│
└── （无 src/backend/api 与 src/backend/engines 的新增入口）
...
|---|---|---|---|
| `PostPrSupervisorConfig` 三处同步 | `rg -n "key_paths\|max_diff_chars\|previous_findings_injection_enabled" src/backend/core/shared/models/ src/backend/infrastructure/config/ src/backend/engines/agent_runner/` | domain dataclass、Pydantic settings、factory 映射三处都有字段，default 一致 | 参考 `feedback-avoid-hardcoding` 记忆：factory 漏映射会被相同默认值掩盖 |
...
| 截断逻辑唯一落点 | `rg -n "6000\|max_diff_chars" src/backend/core/use_cases/pr_supervisor.py` | 只有分层 diff helper 里出现字符预算常量 | 若仍有多处硬编码 6000，说明替换不彻底 |
| 现有测试不回归 | `uv run pytest -o addopts="" tests/` | 全绿（2026-09-16 基线 2091 passed） | 检查 testmon 增量陷阱：`addopts=""` 强制全量 |
...
flowchart TD
    ...
    D -->|通过| F[run_verifier_gate（默认已开,见 c3509c1）]
    F --> F2{verdict}
    F2 -->|red| R3[打回 builder recovery loop]
    F2 -->|yellow/green| G[进 supervisor]
...
- id: rv-3
  behavior: ~~verifier_enabled 默认值为 True~~ **已移除**：该默认值已于 2026-07-07 随 `c3509c1` 落地
    （`ValidationConfig.verifier_enabled` 现为 `True`），不再是本 PRD 的验收项。编号保留空缺。
...
- id: rv-7
  behavior: 现有测试套件不回归(2026-09-16 基线 2091 测试全绿)
  real_entry: "cd /Users/zata/code/keda && uv run pytest -o addopts=\"\" tests/"
  expected: "exit 0;tests 数 >= 2091(2026-09-16 基线) + 新增数(约 9 个)"
  mock_boundary: "无,全真跑"
...
- Depends on tasks/issues:
  - none
- Gate type: none
- Notes: 本 PRD 是 `realistic-validation-independent-verifier-gate` (P1-FEAT-20260628-041733, 已归档) 的后续补丁,非依赖关系(可以独立执行)。初版曾提到"先让用户用上 verifier_enabled 默认 True 再加 stdout 断言"的拆分选项；该默认值已于 2026-07-07 落地，本 PRD 现只包含补丁 1 / 3 / 4。
...
- FR-3 ~ FR-5:**已移除**（原属补丁 2，`verifier_enabled` 默认 `True` 已于 2026-07-07 随 `c3509c1` 落地，
  verifier telemetry 的立项动机随之消失；见 §13 D-11）。编号保留空缺，以免与历史讨论错位。
...
- 不强制要求下游 iar 用户立即启用新配置(补丁 2 移除后，本 PRD 不再有默认值变更类行为变化)
...
- **风险 1**（2026-09-16 已失效，保留作历史记录）: verifier 默认开启后,旧仓升级时 PR 处理时间增加 30%~200%。**现状**:默认值变更已于 2026-07-07 落地(`c3509c1`)，本 PRD 不再包含该项，故该风险不再由本 PRD 引入；`verifier_enabled = false` 的 opt-out 仍然可用
...
| # | 决策问题 | 选择 | 放弃的方案 | 理由 |
|---|---|---|---|---|
| D-01 | 补丁 2 verifier 默认值策略（2026-09-16 已被 D-11 取代）| 直接改默认 `True` + 加 telemetry | 加 .iar.toml 开关灰度 | 用户确认:影响面可控,telemetry 同步观察;灰度路径在小用户群体下收益有限 |
...
| D-07 | 是否引入新 telemetry 框架（已随补丁 2 移除，见 D-11）| 否,用 dataclass 内 dict 计数器 + logger | Prometheus / OpenTelemetry | iur 不是 SaaS,简单内存计数够用;避免新依赖 |
