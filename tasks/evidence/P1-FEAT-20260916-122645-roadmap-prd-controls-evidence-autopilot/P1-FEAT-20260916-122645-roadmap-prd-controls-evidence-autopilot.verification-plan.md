...
| rv-6 全仓质量门禁 | lint / 全量测试 / console 构建 / e2e / 文档构建 | `CI=true just test all`（2368 passed）+ `just lint --reuse` + `pre-commit --all-files` + `just console-sync` + `just e2e …`（17 passed，含 #147 的 lifecycle spec）+ `uv run mkdocs build --strict` | 见 evidence report §3 |
