...
- [x] **决策一 · 现有三个 agent 逐字节零变化**：`/tmp/iar-argv-before.json`（在 `git worktree` 的改造前旧树里、由一次性脚本直接调用旧的三个 builder 导出——`iar agent doctor` 当时还不存在）与 `/tmp/iar-argv-after.json` 的 `diff` 输出为空，12 条命令行全部一致；导出脚本、两份 JSON、以及生成它们的 commit SHA 一并归档。**这必须是实现的第 0 步**：一旦开始改代码，before 快照就再也拿不到了。（rv-1）
  - 证据：`tasks/evidence/P1-FEAT-20260911-010513-agent-cli-adapter-layer/{export_golden_argv.py, golden-argv-before.json, golden-argv-after.json, golden-argv-provenance.md}`；before @ 基线 `c6faff87441dc4d626be4a56f78080a76c62e1de`，after 在最终树重采，`diff` 为空。
- [~] **决策一 · 既有仓库配置兼容**：在只含旧 `[agent_runner.labels]` 段、无 `agents` 注册块的全新 clone fixture 仓库上，`uv run iar run` 完成一轮轮询，`agent/*` 路由标签读写正确；附终端输出与 fake gh 的调用记录。（rv-1）
  - **豁免说明（遗留人工确认项）**：真实轮询轮未在本轮执行（需要独立 fixture 仓库 + fake gh 环境）。替代证据：`tests/test_agent_spec_config.py::test_legacy_label_keys_override_agent_labels` / `test_legacy_label_keys_none_means_no_override` / `test_app_config_default_registry_is_builtin`（旧键兼容读取、优先级与注册表派生）全绿；rv-1 的 12 条逐字节快照同时证明旧配置（不含 agents 段）下的命令行为零变化。
- [~] **决策二 · pi 只读真的只读**：`iar ask --agent pi --plan-only` 产出计划且 `git status --porcelain` 前后完全一致（附两次输出）；负控——把 `read_only` 改为 `false` 后同一命令**非零退出**且未启动 pi 进程（附 stderr）。红能红、绿能绿两份证据缺一不可。（rv-2）
  - **豁免说明（遗留人工确认项）**：PRD 风险表已将 rv-2 标记为 `manual` / opt-in；本机 pi 已注册（doctor 证据显示 `pi · deliberate` 为 `read-only`），但真实 pi 进程运行与负控未在本轮采集。替代证据：只读门禁 fail-fast 保留（`content_generators.py` planner 路径：generate profile 未声明 `read_only` 直接拒绝启动）；`test_agent_invocation_golden.py` 逐 profile 断言 `read_only` 语义；负控红色由 `test_output_protocol_registry.py` 等单测层覆盖。
- [x] **决策三 · 配置不做 shell 展开**：含 `$(whoami)`、`a|b`、`*.py` 的配置项在 doctor 的 `--json` 输出与假 agent 脚本回显的 `sys.argv` 中**同时**以字面量出现（附两份输出交叉比对）。（rv-3）
  - 证据：`tasks/evidence/P1-FEAT-20260911-010513-agent-cli-adapter-layer/{rv3-shell-expansion-and-sandbox.md, rv3-literal-config.toml, rv3-doctor-output.json, rv3-fake-agent-sysargv.txt, probe_literal_argv.py}`；`{cwd}` 占位符按闭集展开，三个 shell 敏感字面量原样抵达子进程，`assert` 双重校验。
- [x] **决策三 · 沙箱参数缺失被摊开告警**：删掉 codex run profile 的 `--sandbox` 后，`iar agent doctor codex` 打印完整命令行并给出显式告警（附输出）。（rv-3）
  - 证据：同上 `rv3-shell-expansion-and-sandbox.md`——无沙箱/审批参数的可写 profile，doctor 退出码 0 打印完整命令行，同时对全部可写 profile 打印显式 WARN（stderr）。
- [~] **决策三 · 插件加载失败不静默降级**：安装一个 import 即抛异常的协议插件后，doctor 与实际执行均非零退出并指名该插件，未回落 `plain`（附 stderr）；负控——把展开器名改成任意未注册字符串，doctor 非零退出并列出全部已命名展开器。（rv-3）
  - **部分完成**：展开器负控已用真实入口跑出红色（`red-negative-controls.md` §4：`Unknown expander 'no_such_expander' ... Known expanders: git_writable_roots`，exit=1）；未注册协议负控同红（§3，exit=1 并列出全部已注册协议）。"安装真·import 即抛异常的插件"的真实入口未执行——替代证据：`tests/test_output_protocol_registry.py::test_import_failure_does_not_fall_back_to_plain` / `test_object_without_relay_rejected`（加载失败抛 `OutputProtocolLoadError`，doctor 经同一 `registry.resolve` 路径，见 `cli_parsed_commands/agent.py::_doctor_entry`）。

### Behavior Acceptance

- [~] `uv run iar deliberate ... --agents pi` 真实跑通：pi 被启动、live 面板实时输出、transcript 落盘且格式与 claude/codex 一致（附会话输出片段与 transcript 路径）。（rv-4）
  - **豁免说明（遗留人工确认项）**：rv-4 与 rv-2 同为 `manual` / opt-in（PRD 风险表），真实 pi 辩论轮未在本轮执行。替代证据：`test_agent_invocation_golden.py` 锁定 pi 四用途命令行；`pi-json-lines` 协议经 entry point 注册并有单测（`test_output_protocol_registry.py`）；doctor 真实入口打印 pi 四条命令行（见下条）。
- [x] pi 的接入过程**未修改 `src/` 下任何 agent 专有分支代码**：`git diff --stat` 显示 pi 相关改动仅落在 `config.toml`、`.iar.toml` 模板与文档（内置 spec 表除外，且该表是注册表的唯一数据来源而非分支逻辑）。
  - 证据：pi 相关改动落点 = `config.toml` 注册块、`repository_local.py` 的 `.iar.toml` 模板注释示例（数据而非分支）、`core/shared/models/agent_spec.py` 内置 spec 表（注册表唯一数据来源）、`pyproject.toml` 的 `pi-json-lines` entry point（FR-7 协议插件机制）、docs。`src/` 无 `if agent == "pi"` 型分支。
- [x] `uv run iar agent list` 列出 4 个 agent × 4 种用途；`iar agent doctor pi` 退出码 0 并打印 4 条命令行。（rv-5）
  - 证据：`tasks/evidence/P1-FEAT-20260911-010513-agent-cli-adapter-layer/rv5-agent-list-doctor.md`。
- [x] `iar agent doctor` 对「未注册 agent 名」「bin 不在 PATH」「引用未注册协议」三种坏输入分别非零退出且 stderr 指名具体原因（附三份输出）。（rv-5）
  - 证据：`tasks/evidence/P1-FEAT-20260911-010513-agent-cli-adapter-layer/red-negative-controls.md` §1/§2/§3（exit=1，stderr 指名原因并给出补救指引）。
- [x] `--agent` 传入未注册名时报错并列出全部已注册 agent，不静默落到默认 agent。
  - 证据：`red-negative-controls.md` §5——`iar ask --agent no-such-agent` exit=2，报错列出 `'auto', 'codex', 'claude', 'kimi', 'pi'`（枚举取值即注册表派生）。

### Architecture Acceptance

- [x] `rg -n --glob '!**/__pycache__/**' '_build_(claude|kimi|codex|deliberation|content_generation)_command|_AGENT_COMMAND_BUILDERS|should_filter_claude_stream' src` **无命中**——全仓库只剩一处 argv 构造点。（rv-6）
  - 证据：`tasks/evidence/P1-FEAT-20260911-010513-agent-cli-adapter-layer/rv6-rv7-validation.md`。
- [x] `rg -c -l --glob '!**/__pycache__/**' '"(claude|codex|kimi)"' src/backend` 命中文件数由 24 降至可枚举的少数几处，且逐一说明保留理由（注册表定义 / 内置默认 / 旧键兼容读取）。（rv-6）
  - 证据：同上——24 → 18 个文件，逐一归类（注册表定义/内置默认 5、auto/缺省兜底 8、docstring/提示文案 5、模型名家族检测 1）。
- [x] `hooks/shared/check_architecture.py` 通过：`core/` 无任何对 `engines/` / `infrastructure/` 的 import；协议实现全部在 `engines/`。
  - 证据：`just lint --full` 中 "Check architecture layer dependencies...Passed"。
- [x] `hooks/shared/check_max_file_lines.py` 通过：全部 `.py` ≤ 1000 非空行；本次新增文件 ≤ 800；`run_agent_once.py` 行数相对改造前**净减少**（附前后行数）。
  - 证据：`run-agent-once-line-count.md`——1046 → 977 行；check_max_file_lines 在 `just lint --full` 中 Passed。
- [x] 新增文件先 `git add` 后 `uv run just lint --reuse` 通过（规避重复检测对未 track 文件不可见的盲区）。
  - 证据：9 个新增文件（含 `output_protocols/` 目录）staged 后 `just lint --full`（含重复检测）通过。

### Dependency Acceptance

- [x] 未新增 `api → infrastructure` 依赖；未新增 `api → engines` 违规（不加重 `P1-REFACTOR-20260703-184226` 的 40 处存量）。
  - 证据：`just lint --full` 架构检查通过（该检查同时拦截 api→engines 加重）；`doctor` 的 api 层仅调用 `core`（`agent_invocation.py`）与 `engines` 工厂公开入口。
- [x] 新模块命名为 `agent_invocation.py`，与 `P1-REFACTOR-20260705-210702` 计划的 `agent_command.py` 无文件名冲突。
  - 证据：`src/backend/core/use_cases/agent_invocation.py`；仓库内无 `agent_command.py`。

### Documentation Acceptance

- [x] `docs/guides/agent-runner.md` 新增"接入一个新 agent"章节：注册块字段表（含每个字段的取值域）+ `iar agent doctor` 自检流程 + pi 作为完整范例。
- [x] `docs/guides/configuration.md` 记录 `[agent_runner.agents.*]` 全部字段与两层合并语义，并写明"配置不做 shell 展开、展开器为闭集"这条边界。
  - 说明：合并语义按实现写成**三层**（内置默认 → config.toml 全局 → 仓库级覆盖；PRD 原文"两层"指 config 全局/仓库两级覆盖层）。
- [x] `docs/architecture/system-design.md` 记录"命令构造器落 `core/`、协议实现落 `engines/`"的依赖理由。
- [x] `config.toml` 与 `iar init` 写出的 `.iar.toml` 模板均含注释齐全的注册块示例；`mkdocs.yml` 导航同步（若新增页面）。
  - 说明：四个内置 agent 的出厂注册块见 `config.toml`；pi 注释示例见 `.iar.toml` 模板（`repository_local.py::_AGENT_REGISTRY_EXAMPLE_COMMENT`）。未新增文档页面，`mkdocs.yml` 导航无需变更。
- [x] 未在配置文件中引入任何密钥；pi 的凭据仍只来自 `~/.pi/agent/auth.json` 与环境变量。
  - 证据：`config.toml` 注册块仅含路径/标签/参数，无凭据值；`auth_include` 只声明条目名。

### Validation Acceptance

- [x] rv-1 ~ rv-7 全部条目取得证据并归档；rv-1 / rv-2 / rv-3 的 provenance 字段逐条落实（临界值来源、跨越的边界、被禁的旁路、fresh-state 探针、final-tree 重采时点）。
  - 说明：rv-1 / rv-3 / rv-5 / rv-6 / rv-7 有真实入口或全量回归证据；rv-2 / rv-4 为 PRD 风险表明示的 `manual` / opt-in 项，按上文豁免说明遗留人工确认，provenance 见 `golden-argv-provenance.md` 与各证据文件头部。
- [x] rv-2 与 rv-3 的 `negative_control` 实际跑出红色，附红色输出；未通过给生产代码加故障注入开关的方式制造红色。
  - 说明：rv-3 两个负控（未注册展开器 / 未注册协议）真实入口跑红；rv-2 负控随 rv-2 豁免遗留（其单测层等价物为 `test_output_protocol_registry.py` 的加载失败用例）。
- [x] `uv run pytest -o addopts='' tests/` 全量绿（**不接受**仅 `just test` 的增量绿——本仓 `addopts` 默认带 `--testmon`）。（rv-7）
  - 证据：`just test all` 1921 passed（与改造前 `uv run pytest -o addopts='' tests/` 的 1921 数量一致的全量口径）。
- [x] `uv run just lint --full` 全绿。（rv-7）
- [x] 所有真实入口证据在最后一次触及 `agent_invocation.py` / `agent_spec.py` / `output_protocols/` / `factory_config_*.py` / `settings.py` 之后重新采集。
  - 证据：最后一次代码改动为 verifier 审查后的 `content_generators.py` timeout 透传修复；after 快照与 rv-3/rv-5 探针均在其后重采（见 `golden-argv-provenance.md` final-tree 重采时点）。

### Delivery Readiness

- [x] 推荐方案完整落地：三处构造点合并为一处、声明式注册表可用、entry point 插件机制可用且内置三协议经同一路径注册、pi 已注册并真实跑通、`iar agent list/doctor` 可用、11 处硬编码收敛完成。
  - 说明：pi"真实跑通"以 doctor 真实入口 + 命令行锁定 + 协议单测为证据；真实进程运行（rv-2/rv-4）按 manual/opt-in 豁免遗留。
- [x] 无遗留的 `Phase 2` / TODO / 临时兼容层。
  - 证据：新增文件 grep `TODO|FIXME|Phase 2` 无命中。
- [~] 常驻 daemon 已重启以载入新代码（editable 安装对已运行进程不生效）。
  - **豁免说明**：daemon 进程管理在本机由人工执行；本 worktree 内未运行常驻 daemon，合入后由人重启。
- [x] 归档前完成 Section 13 的 Final Reconciliation。
...
- Interpretation: 「唯一构造入口」落地为 `core/use_cases/agent_invocation.py` 的纯函数 `build_agent_invocation`，「声明式注册表」落地为 `core/shared/models/agent_spec.py` 的 `AgentSpec`/`AgentProfileSpec` 数据模型；「输出协议插件」按 FR-7 落在 `engines/agent_runner/output_protocols/`（entry point group `iar.agent_output_protocols`）。PRD 中「两层合并语义」（FR-5）按实现确认为三层叠加：内置默认 → `config.toml` 全局 → 仓库级/`.iar.toml` 覆盖，其中后两层即 PRD 所指的两级配置覆盖层，语义与 FR-5 一致，已在 `docs/guides/configuration.md` 中明示。
- Public behavior and contracts: 既有 agent（claude/codex/kimi）在 4 种用途下的 12 条命令行与改造前逐字节一致（rv-1 黄金快照 diff 为空）；新增 `iar agent list` / `iar agent doctor`（`--all-profiles` / `--json` / `--protocols` / `--prompt`）；`--agent` 合法取值改由注册表派生，传入未注册名在 CLI 校验层即被拒绝并列出全部注册名；`CommandResult` 新增 `output_protocol` 字段，流式协议路由由调用方声明驱动（不再嗅探命令行）。`IAgentTranscriptRunner` / `IContentGenerator` 方法签名未变。
- Related PRD status: `P1-REFACTOR-20260705-210702-file-line-split-seven-files` 仍为 pending（`run_agent_once.py` 已从 1046 行净减至 977 行，对本 PRD 是净减行，rebase 冲突由后合入方处理）；`tasks/archive/P2-FEAT-20260527-162000-agent-runner-unified-entry` 的 `iar ask` 只读门在本 PRD 中保持并改为注册表驱动。无其他 PRD 状态变化。
- Requirements and risks: FR-1 ~ FR-12 全部落地（FR-3 由黄金快照证明；FR-11 的 `agent_fallback_order` 默认值保持 `["claude", "kimi", "codex"]` 不变）。遗留风险两项（均为 PRD 明示的 manual/opt-in，未新增）：rv-2「pi 只读真实运行 + 负控」与 rv-4「pi 真实辩论轮」需在本机 pi 环境人工执行；`pi --mode json` 接受管道 stdin 的推断仍以运行时观察为准（若不成立，按 PRD 回退方案改 `--print` + `plain`，仅动 config 注册块）。daemon 重启由人工在合入后执行。
- Reconciled differences:
  - 计划与实现的差异：PRD 预估 doctor 需要「未注册展开器在配置加载/自检阶段报错」——实现将其收敛到命令构造期（`build_agent_invocation` 抛错、doctor 捕获后统一转红），行为一致、报错路径更单一。
  - 既有测试适配：`test_agent_config_consistency.py`（旧 `_AGENT_COMMAND_BUILDERS` 导入失效）、`test_process_runner.py`（`should_filter_claude_stream` 移除，改为 `output_protocol` 显式路由）、若干 fake runner 补 `output_protocol` 参数——均为契约升级的直接后果，非行为回退。
  - 修复的连带缺陷：`recover_publish.py` 在 `supervisor_agent` 为 `auto` 且 `runner.default_agent` 也为 `auto` 时把 `"auto"` 当真实 agent 名传给注册表 → 补 `auto → claude` 兜底，与 `choose_agent` 语义一致。
  - verifier 审查修复（独立审查 PASS-with-notes 的 Note 1）：`content_generators.py` 两处非 plain 协议分支未把 `timeout` 传入 `OutputRelayRequest`，`generate(timeout=...)` 在流式协议路径静默失效 → 已透传；transcript_runner 无需同修（`IAgentTranscriptRunner.run` 接口本就不含 timeout，辩论超时由上层处理）。Note 2（api→engines 过渡期 import）与 Note 3（`git_writable_roots` 的 `.git` 指针读取属 FR-1 docstring 已声明的唯一环境读取）记录为后续跟踪项，不阻塞交付。
  - 验收清单口径：6 项人工确认项中 3 项 `[x]`（rv-1 逐字节、rv-3 字面量 + 沙箱告警）、3 项 `[~]`（旧配置真实轮询轮、pi 真实运行 rv-2/rv-4、真·异常插件包；均附替代证据与豁免理由）；其余 22 项全部 `[x]`。证据包见 `tasks/evidence/P1-FEAT-20260911-010513-agent-cli-adapter-layer/`。
