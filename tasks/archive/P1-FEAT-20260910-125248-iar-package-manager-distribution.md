...
> 本清单是唯一完成门禁。带**（发布时执行）**前缀的条目依赖首次 PyPI 发布 / GitHub
> release 事件 / tap 维护者动作，在首次发布前无法产出真实证据，因此保持未勾选——
> 它们未完成即代表本 PRD 尚不可归档。执行顺序见 `docs/guides/release-process.md`。
...
- [x] **包名与首次发布（rv-1，2026-09-11 实测）**：`https://pypi.org/pypi/kedacode/json` 返回 200，name=`kedacode` version=`0.2.0`，2 个文件；全新 3.12 venv `uv pip install kedacode==0.2.0` 后 `iar 0.2.0`，`iar console` 起服返回 `/` 200、`/api/v1/agent-runner/health` 200 JSON、`/app/dashboard/` 200。产物来源由 PyPI JSON API 的 `urls[].url` 佐证，两个文件均在 `files.pythonhosted.org/packages/...`。**反向验证已在发布前采集**：打完 tag、draft release 已存在时 `pypi.org/pypi/kedacode/json` 与 `.../0.2.0/json` 均为 **404**（这一刻过后不可复现）
- [x] **发布凭据：仓库内零长期密钥 + OIDC 真实上传（rv-2 主体，2026-09-11 实测）**：`gh secret list` 只有 `HOMEBREW_TAP_TOKEN`，`gh api repos/ZataZhang/keda/environments/pypi/secrets` 无任何条目，`rg -ni 'pypi.?token|PYPI_API_TOKEN|twine.*password|__token__' .github/ install.sh` 无匹配（退出码 1）；`publish-pypi` job（`environment: pypi` + `id-token: write`）在 `release: published` 事件下 **completed/success**，`kedacode 0.2.0` 随即出现在 PyPI，且 PEP 740 provenance 端点返回 200——attestation 只有 Trusted Publishing 才拿得到，用 token 发布不会有
- [x] **发布凭据：无-token 断言能判负（rv-2 反向验证 b 段）**：把 `release.yml` 复制到临时文件并追加一行 `password: ${{ secrets.PYPI_API_TOKEN }}`，对副本跑同一条 `rg` **命中第 347 行**；仅操作临时副本，未改仓库配置
- [x] **发布凭据：OIDC 链路的反向验证（rv-2 反向验证 a 段，2026-09-13 实测）**：用临时 workflow `oidc-negative-control.yml` 跑两档对照，证据采到后该文件已删除（提交 `7ce448d` → `d4cf61a`）。**反向档**（`permissions: contents: read`，不带 `id-token: write`）：`ACTIONS_ID_TOKEN_REQUEST_URL` 与 `ACTIONS_ID_TOKEN_REQUEST_TOKEN` 双双 **未注入**（打印 `is set: no`），`pypa/gh-action-pypi-publish` 步骤 **outcome=failure**，断言步骤据此判定通过。**正向对照档**：同一仓库、同一动作，只多 `id-token: write` 一项权限，两个变量均注入、OIDC 签发请求返回 **HTTP 200**、令牌长度 1931——证明反向档的失败确由缺该权限造成，而非别的环节。不可能误上传的两个独立理由：无该权限时 runner 拿不到签发端点且仓库无任何 PyPI token secret（`gh secret list` 仅 `HOMEBREW_TAP_TOKEN`，`environments/pypi/secrets` 计数为 0）；PyPI 的 Trusted Publisher 登记绑定 `release.yml` + environment `pypi`，临时 workflow 两者皆不是
### Behavior Acceptance
...
- [x] **rv-3（2026-09-11 实测，Apple Silicon / macOS 26.6.2）**：先 `brew uninstall` + `brew untap` 回到干净态，`brew install ZataZhang/tap/kedacode` 成功——8 分钟源码构建，1746 files / 26.5MB；`iar --version` → `iar 0.2.0`；`xattr -p com.apple.quarantine $(brew --prefix)/bin/iar` → `No such xattr`；`iar console --no-browser` 起服返回 `/` 200、`/api/v1/agent-runner/health` 200 JSON、`/app/processes/` 200——**这条是 32 个 resource 的传递依赖完整性的功能级证明**（漏依赖的表现正是装得上、一跑就崩）。验证后已 `brew uninstall` + `brew untap` 恢复环境。
  首次尝试失败并暴露两个真缺陷，均已修：formula 未声明 Rust 构建依赖（依赖树含 `pydantic-core`，无 Rust 时构建必败）→ 补 `depends_on "rust" => :build`，Homebrew 随后自动装 rust bottle 即通过；Homebrew 对源码构建失败会追加"CLT 有新版"的通用建议，实测是红鲱鱼（本机 clang 21.0.0 正常，26.6.0 已是 macOS 26.6.2 的最新 CLT）
- [x] **rv-4（2026-09-11 实测）**：推送 `v0.2.0` 后 `gh release view` 显示 `isDraft=true` 且三个资产齐全，同一时刻 `pypi.org/pypi/kedacode/json` 与 `.../0.2.0/json` 均 **404**——tag 确实没有发布任何东西；`gh release edit --draft=false` 之后 `publish-pypi` 触发，PyPI 转为 **200**。两半均为真实 HTTP 状态码，非读 workflow 推断
- [x] **同版本重复发布的幂等性（2026-09-13 实测）**：0.2.1 已上线后重跑 `publish-pypi` job（run 34751739025，job 103709341391），twine 输出 `Skipping kedacode-0.2.1-py3-none-any.whl because it appears to already exist` 与 `Skipping kedacode-0.2.1.tar.gz because it appears to already exist`，动作 **outcome=success / conclusion=success**——`skip-existing: true` 确实把重复上传处理成幂等成功而非失败
- [x] **rv-5 真实档（2026-09-13 实测，macOS 26.6.2）**：两个阻塞缺陷均已修复后端到端跑通。先验拦截：旧包名 `keda` 的 editable 安装在位时，`bash install.sh --source pypi --method uv` 被 `check_legacy_tool_conflict()` 拦下并给出确切命令（`uv tool uninstall keda`），而不是抛 uv 那句 `Executable already exists: iar`。`uv tool uninstall keda` 后重跑：**exit 0**，安装完打印 `iar 0.2.0`。功能级探活在**临时目录** `/tmp/rv5probe`（非仓库 checkout，证明静态资源确实随 wheel 分发）：`/` 200、`/api/v1/agent-runner/health` 200 且返回 `{"status":"healthy","gh_cli_available":true}`、`/app/dashboard/` 200、`/app/processes/` 200；`lsof` 确认仅监听 `127.0.0.1:8791`。验证后已 `uv tool install --reinstall --editable .` 恢复开发用安装（现注册名为 `kedacode v0.2.1`，同时提供 `iar` 与 `kedacode` 两个可执行文件）
- [~] **无 release 场景（窗口已关闭，降级为静态+URL 证据）**：`v0.2.0` 发布后本仓已不可能回到"零 release/tag"状态，该运行时场景不可复现。保留的证据：`rg -n "refs/tags" install.sh` 证明 tag 分支与 main fallback 分路，fallback 拼的是 `archive/refs/heads/main.tar.gz`；该 URL 实测返回 **200**，而修复前的 `archive/refs/tags/main.tar.gz` 实测 **404**（FR-12）。想跑端到端需另建一个零 release 的仓库并 `KEDA_REPO` 指过去，且同样绕不开上一条的 python 门禁
### CI Acceptance
...
- [x] **rv-6 真实安装档（2026-09-11 实测）**：`install-smoke.yml` 在 main 上 success，job 列表含 `Real install + console probe on ubuntu-latest / macos-latest / windows-latest` 三条**全部 success**，走的是真实 `uv tool install` + `iar --version` + console 探活，不是 `--check`
- [x] **rv-6 公共 PyPI 档（2026-09-13 实测）**：`Install from public PyPI + console probe` 在 **ubuntu-latest / macos-latest / windows-latest 三平台全部 success**（run 34751738974，总 conclusion success），走的是 `uv tool install kedacode==0.2.1` 真实安装 + 起 `iar console` 后 curl 探活。过程如实记录：v0.2.1 首轮发布时 macOS 与 Windows 直接绿，ubuntu 两档红，重跑后全绿；**重跑用的是当时的 workflow 文件**，能通过是因为 PyPI 传播早已完成——它证明的是本条主张（三平台可从公共索引装上并探活），竞态本身的修复见下一条
- [x] **tap 写入凭据范围（2026-09-11 由维护者创建并核验）**：专用 fine-grained PAT `keda-release-homebrew-tap`，Resource owner=`ZataZhang`，Repository access=**Only select repositories** 且仅勾 `ZataZhang/homebrew-tap`，Repository permissions 仅 **Contents: Read and write**（维护者截图确认），有效期 366 天；未复用任何具备 keda 仓库写权限的 token。功能级佐证：初次仅设了仓库范围未加权限时 `update-tap` 推送以 `Permission to ZataZhang/homebrew-tap.git denied` 403 失败，补上 Contents 写权限后同一 job 一次通过
- [x] **update-tap 首次运行（2026-09-11 实测）**：推送前 `gh api repos/ZataZhang/homebrew-tap/contents` 返回 `This repository is empty`（无任何 commit、`main` 不存在）；job 成功建分支并推出 `Formula/kedacode.rb` 与 `README.md`（tap 侧 commit `9155589 kedacode 0.2.0`），`gh api .../contents/Formula/kedacode.rb` 可读
### Dependency Acceptance

- [x] `update-tap` job `needs: publish-pypi`，PyPI 上传失败时不会产生指向不存在版本的 formula（workflow 第 151 行）


- [x] **formula resource 对齐（2026-09-11 实测）**：`Formula/kedacode.rb` 含 **32 个 resource 块**，全部为 `files.pythonhosted.org` 的 sdist，由 `brew update-python-resources` 从 PyPI 元数据生成（因此与该版本的运行时依赖同源）；功能级证明见 rv-3——装完 `iar console` 能起服返回 200。生成方式与"resource 块由 CI 生成、勿手改；`depends_on` 等非 resource 行手工维护"已写入 tap README。
  注：0.2.0 的 formula 仍含 `uvicorn[standard]` 带入的 uvloop / httptools / websockets / watchfiles 四个编译型 resource；`uvicorn[standard]` 已降为 `uvicorn`，下个版本的 formula 会相应变薄（运行时闭包 68 → 29，编译型依赖 5 → 1）
- [x] **推空 formula 的可能性已被门禁堵死**：`brew tap <user/repo> <path>` 是 clone，`update-python-resources` 改写的是 Homebrew 自己的 tap 副本——workflow 已把结果 `cp` 回待推送工作树，并新增 `Verify the formula declares its dependencies` 步骤：`grep -c '^  resource "'` 小于 1 即 `exit 1`。没有这道门禁时，job 会静默绿灯并推出一个装得上、一跑就崩的 formula
...
- [x] **oracle 执行状态（2026-09-11 回填）**：rv-1 / rv-3 / rv-4 / rv-7 / rv-8 已执行并留真实证据；rv-2 主体（零长期密钥 + OIDC 真实上传 + attestation）与 b 段反向验证已完成，a 段推迟到 0.2.1；rv-6 真实安装三平台已绿，公共 PyPI 档因竞态失败、已修待下个版本；rv-5 被 install.sh 的 python 门禁挡住（见 Risks）
- [x] **反向验证（rv-1 已完成，rv-2 一段完成一段推迟）**：rv-1 的红在正式发布**之前**采到——tag 已推、draft release 已存在时 PyPI 对包与版本均 404，发布后转 200，这个顺序不可倒推；rv-2 的 b 段（对插入 token 行的临时副本跑 `rg` 必须命中）已确认可转红；a 段（不带 `id-token: write` 时 publish job 必须失败）推迟到 0.2.1，理由见 Human-Confirmed 段
- [x] **证据来源合规（2026-09-11 复核）**：R2/R3 证据全部取自真实产出——PyPI JSON API 的真实响应与 `urls[].url`（`files.pythonhosted.org/packages/...`）、`gh release view` 的 `isDraft` 字段、两次 `curl` 对 PyPI 的真实 HTTP 状态码（404 → 200）、`brew install` 的原始输出与 `xattr` 输出、`gh secret list` 与 `gh api .../environments/pypi/secrets` 的真实返回、CI job 的 conclusion 与日志。无一条来自读 workflow 文件或源码常量自述
### Delivery Readiness
...
| **改名后旧包名 `keda` 与 `kedacode` 的可执行文件冲突**（2026-09-11 rv-5 实测发现） | `keda` 与 `kedacode` 都注册 `iar`，uv 拒绝让不同包覆盖同名 binary：`error: Executable already exists: iar (use --force to overwrite)`。**任何装过旧 `keda` 的用户都会撞上**，而 uv 那句报错既没说冲突来自哪个包、也没说怎么办。install.sh 的失败传播是正常的（退出码 2，不静默） | 已修：`check_legacy_tool_conflict()` 提前拦下并给出确切命令（`uv tool uninstall keda` 后重跑），`--check` 模式只告警不退出以便 dry-run 完整打印计划；并特别提示"若该条目是本仓库的 editable 开发安装，请保留它并跳过安装器"。rv-5 的端到端仍待在无旧包的干净环境采集（CI 的 `stock-python` 档已覆盖该路径） |
| **`install.sh` 的 python 门禁让一键安装在原装 macOS 上必然失败**（2026-09-11 发现） | `detect_python()` 读 ambient `python3` 并在 bootstrap uv **之前** `exit 1`；macOS 自带 3.9.6，于是 README 顶部那条 `curl … \| bash` 对任何没自行装过 Python 的 Mac 用户直接报 `Python 3.9.6 is too old`。走 uv 路径时 ambient python 根本无关——uv 会自己准备解释器。CI 之所以没暴露：`install-smoke` 先跑 `actions/setup-python` 才调 install.sh，把这个前提掩盖了 | 已修：`detect_python()` 改为只记录状态不退出（`PY_OK`），版本门禁下移到 `resolve_installer` 且**只对 `pipx` / `pip` 生效**；ambient python 过旧或缺失时自动选 uv 并说明原因。实测三场景：3.9.6 下默认走 uv（退出码 0）、`--method pip` 明确报错（退出码 1）、连 `python3` 都没有时也能走 uv。同时给 install-smoke 加了 `stock-python` 档（刻意装 3.10 复现 `< 3.11`），否则这类问题会继续被 `actions/setup-python` 掩盖 |

| **tap formula 永远钉在首次 bootstrap 的版本**（2026-09-13 发布 0.2.1 时发现） | `update-tap` 全绿而 tap 上主 url 仍是 `kedacode-0.2.0.tar.gz`，用户 `brew install` 装到旧版本；0.2.0 时不可见，因为那次正好是 bootstrap | 成因是两个各自看起来都对的步骤相加：Bootstrap 只在 formula 不存在时才写，`update-python-resources` 只重写 resource 块、不碰主包 url。已修：新增 Pin 步骤每次按 `KEDACODE_VERSION` 改写主包 url/sha256，Seed 步骤改为也提交该改动（`brew tap <repo> <path>` 是 clone，只认已提交内容），并加门禁——主 url 与目标版本不符即失败。实测修复后 tap 更新到 0.2.1，resource 由 32 降至 28、4 个编译型依赖消失，与本 PRD 预测的依赖瘦身逐字吻合 |
| **install-smoke 与 PyPI 传播赛跑，且等待轮错了索引**（2026-09-13 发现） | 0.2.1 首轮两个 ubuntu 档红：公共 PyPI 档的等待轮 JSON API，60 秒返回 200 后 `uv` 仍 0 秒失败于 `no version of kedacode==0.2.1`——`uv`/`pip` 解析的是 simple 索引，两者缓存不同步；stock-python 档则根本没有等待 | 已修：抽出 `.github/actions/wait-for-pypi` 改轮 simple 索引，原 bash / pwsh 两份重复实现合并为一份（Windows runner 自带 Git Bash），并给 stock-python 档补上这道等待。本地实测 v0.2.1 命中 exit 0、99.99.99 超时 exit 1（能判负） |
| **上游 Homebrew 删除 `master` 分支**（2026-09-13 发现，上游侧时间性断裂） | `Homebrew/actions/setup-homebrew@master` 无法解析，`update-tap` 在 Set up job 阶段即失败；0.2.0 发布时同一引用还能解析 | 已改 `@main`（上游默认分支已是 main，仓库里留着 `remove-master-sync` 分支为证） |
| **Homebrew 6.x 的 tap trust 会挡住 `brew tap` 本身**（2026-09-13 发现） | `Refusing to load formula ... from untrusted tap` 导致 `Cannot tap ...: invalid syntax in tap!`；与 Risks 里原有那条（面向终端用户的短名安装）是同一机制的不同触发点 | 已修：在 `brew tap` **之前**执行 `brew trust --formula ZataZhang/tap/kedacode`。两个细节都踩过坑：放在 tap 之后压根轮不到执行；用 `--tap` 无效（本地 Homebrew 6.0.22 实测 `Trusted tap:` 打印成功后 `brew tap` 照样拒绝，换 `--formula` 后立刻 `Tapped 1 formula`），尽管 Homebrew 自己的报错把两条都列了出来 |

**Follow-ups（不阻塞本 PRD）**：

- **上游模板仓的旧用户名**：`scripts/shared/template/sync_template.sh` 的 `TEMPLATE_REPO` 仍指向失效的 `zata-zhangtao/zata-codes-template`（靠 GitHub 仓库级重定向还能 clone）。该文件由 `scripts/shared/*` 规则判定为**上游所有**（见脚本自身的 ownership case：`scripts/shared/*|scripts/build/*` 参与同步、`scripts/*` 其余跳过），**不能在 keda 下游手改**——本 PRD 实现时曾改过一次、评审时已还原。正确做法是去 `ZataZhang/zata-codes-template` 上游改，再 sync 回来。`scripts/template/sync_template.sh` 属项目自有，已改。
- 首次启动的环境体检页（git / `gh auth` / agent CLI 检测与修复指引）——对开发者用户价值较低但仍是真实摩擦点，作为独立小 PRD 处理。
- 原生安装包在出现真实的非开发者用户诉求时，按 7.10 记录的事实重新评估。
- `iar console` 的自动更新提示。
...
2026-09-13 在 0.2.1 发布时补完最后 4 条验收项（rv-2 反向验证 a 段、skip-existing 幂等、rv-5 真实档、rv-6 公共 PyPI 档），并据此复核全文。

- Interpretation: confirmed —— 包管理器分发、两步发布（tag 出 draft、人工检查后才上传 PyPI）、仓库内零长期发布密钥三条核心解读均按原样落地，本轮拿到正向与反向两侧证据，无偏离。
- Public behavior and contracts: confirmed —— 包名 `kedacode`、可执行文件 `iar` 与 `kedacode`、三条安装入口（brew / uv·pipx / install.sh）、`release: published` 触发上传，均与 FR 一致并经 0.2.1 实测。唯一新增的对外行为是 `install.sh` 对旧包名冲突的提前拦截（`check_legacy_tool_conflict`），属 0.2.0 遗留缺陷的修复，已在 §12 记录。
- Related PRD status: confirmed —— 上游 `P1-FEAT-20260910-111901`（console 打进 wheel）同批归档，本 PRD 依赖它产出的静态资源；0.2.1 的 tap formula resource 由 32 降至 28、4 个编译型依赖消失，正是该 PRD 依赖瘦身的独立佐证。无新增下游依赖。
- Requirements and risks: corrected —— §12 新增 4 条本轮实测暴露的缺陷（tap formula 版本永不更新、install-smoke 轮错索引且有一档完全没等、上游 Homebrew 删除 master 分支、Homebrew 6.x 的 tap trust 挡住 brew tap），均已修复并留证。其中 tap 版本 pin 是**用户可见缺陷**（会装到旧版本），恰好说明只看「job 绿灯」不够，本 PRD 坚持功能级证据的口径是对的。§10 与 §13 各项决策未被推翻，无需改写。
- Reconciled differences:
  - rv-6 证据口径收敛：三平台绿是**重跑**采到的，重跑用的是当时的 workflow 文件，能通过是因为 PyPI 传播早已完成。它证明的是「三平台可从公共索引装上并探活」，不是「竞态修复已验证」；竞态修复目前只有本地正反两向证据（v0.2.1 命中 exit 0 / 99.99.99 超时 exit 1），CI 级证据要到下个版本发布时才能采到。
  - rv-2 反向验证 a 段的实施方式与原计划不同：原计划是「发布时先跑一次不带 id-token 的 publish job」，实际改用独立的临时 workflow 跑反向档 + 正向对照档，证据采到后删除。这样既不必动已公开的 release 状态，又多了一个只差该权限一项的对照组，判别力更强。
  - rv-5 的两个阻塞项（install.sh 的 python 门禁、旧包名可执行文件冲突）在 0.2.0 之后已修复，本轮端到端跑通，不再是 Risks 中的待办。
