...
- **一处未覆盖在"CLI 逐字节不变"之内的可观测变化（归档后补记）**：搬走的函数各自使用新模块的 ``logging.getLogger(__name__)``，因此日志记录的 **logger 名**随之变化，共 8 条 （``create_or_reuse_worktree`` 2 条：``run_agent_once`` → ``agent_runner_worktree_create``；attempt 记账 2 条：``run_agent_once`` → ``agent_runner_attempt``；4 个 ``_process_*`` 4 条：``agent_runner_orchestrate`` → ``agent_runner_issue_handlers``）。日志**正文与级别不变**，只有归属模块名变化；按 logger 名过滤日志的下游需注意。保持原 logger 名需要硬编码外部模块名或注入 logger，两者都更差，故选择声明而非改写。测试不受影响（``caplog`` 断言基于 message 文本，且捕获所有传播记录）。
